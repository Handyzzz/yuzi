...
-(void)setupTipTitleView{
    ...
    UIView*mask=[[UIView alloc]initWithFrame:CGRectMake(0, _maskView.sd_y+_maskView.sd_height, self.view.sd_width, kAppScreenHeight -_maskView.sd_y - _maskView.sd_height -self.view.sd_height * 0.1)];
...
//    mask.bounds = CGRectMake(0, 0, self.view.sd_width + kBorderW + kMargin , self.view.sd_height * 0.9);
...
- (void)imagePickerController:(UIImagePickerController *)picker didFinishPickingMediaWithInfo:(NSDictionary *)info
{
    ...
    [picker dismissViewControllerAnimated:YES completion:^{
        ...
        //10.3之后的bug， try catch都不管用
        NSArray *features;
        @try{
            
        features = [detector featuresInImage:[CIImage imageWithData:UIImagePNGRepresentation(image)]
                                              options:@{CIDetectorImageOrientation: [NSNumber numberWithInt:6]}];
        }
        @catch(NSException * e){
            NSLog(@"e is %@", e);
            
            UIAlertView * alertView = [[UIAlertView alloc]initWithTitle:@"提示" message:@"该图片存在二维码，但是当前版本的应用未能处理" delegate:nil cancelButtonTitle:@"确定" otherButtonTitles:nil, nil];
            [alertView show];
        ...
        if (features.count >=1) {
            ...
            //todo，判断是加朋友还是加入了群组
            [WYUtility handleAddFriendQrcodeUrl:[scannedResult escapedURL]];
        }
        else{
            UIAlertView * alertView = [[UIAlertView alloc]initWithTitle:@"提示" message:@"该图片没有二维码！" delegate:nil cancelButtonTitle:@"确定" otherButtonTitles:nil, nil];
            [alertView show];
