...
- (void)layoutSubviews {
    if (_isBuildUI) {
        self.bodyScrollView.contentSize = CGSizeMake(self.bodyScrollView.width * [self.viewsArray count],0);
        for (int i = 0; i < [self.viewsArray count]; i++) {
            UIViewController* vc = self.viewsArray[i];
            vc.view.frame = CGRectMake(self.bodyScrollView.width * i, 0, self.bodyScrollView.width, kAppScreenHeight);
        }
    }
}
...
- (void)selectTabWithIndex:(NSUInteger)index animate:(BOOL)isAnimate {
    ...
    //移动select line
    if (isAnimate) {
        if ([self.delegate respondsToSelector:@selector(bodyScrollViewWillBeginDragging:)]) {
            [self.delegate bodyScrollViewWillBeginDragging:self.bodyScrollView];
        }
        [UIView animateWithDuration:0.3 animations:^{
            moveSelectedLine();
        } completion:^(BOOL finished) {
            if (finished == YES) {
                if ([self.delegate respondsToSelector:@selector(bodyScrollViewDidEndDecelerating:)]) {
                    [self.delegate bodyScrollViewDidEndDecelerating:self.bodyScrollView];
                }
                ...
                for (int i = 0; i < self.tabButtons.count; i ++ ){
                    ...
                    button.enabled = YES;
                    if (i == index) {
                        button.titleLabel.font = [UIFont systemFontOfSize:15 weight:0.4];
                    }else{
...
- (void)scrollViewWillBeginDragging:(UIScrollView *)scrollView {
    if (scrollView == self.bodyScrollView) {
        ...
        if (_isEndDecelerating) {
            ...
            if ([self.delegate respondsToSelector:@selector(bodyScrollViewWillBeginDragging:)]) {
                [self.delegate bodyScrollViewWillBeginDragging:scrollView];
            }
        }
        _isUseDragging = YES;
...
- (void)scrollViewDidEndDecelerating:(UIScrollView *)scrollView {
    if (scrollView == self.bodyScrollView) {
        ...
        if ([self.delegate respondsToSelector:@selector(bodyScrollViewDidEndDecelerating:)]) {
            [self.delegate bodyScrollViewDidEndDecelerating:scrollView];
        }
...
- (void)scrollViewDidEndDragging:(UIScrollView *)scrollView willDecelerate:(BOOL)decelerate {
    if (scrollView == self.bodyScrollView && decelerate == NO) {
        if ([self.delegate respondsToSelector:@selector(bodyScrollViewDidEndDecelerating:)]) {
            [self.delegate bodyScrollViewDidEndDecelerating:scrollView];
        }
        [self selectTabWithIndex:(int)scrollView.contentOffset.x/self.bounds.size.width animate:YES];
...
- (UIView *)selectedLine {
    if (!_selectedLine) {
        self.selectedLine = [[UIView alloc] initWithFrame:CGRectMake(0, self.tabView.frame.size.height -3, self.selectedLineWidth, 2)];
        _selectedLine.backgroundColor = self.selectedLineColor;
        [self.tabView addSubview:_selectedLine];
        
        UIView *lineView = [[UIView alloc]initWithFrame:CGRectMake(0, self.tabView.frame.size.height -1, kAppScreenWidth, 1)];
...
- (UIScrollView*)bodyScrollView {
    if (!_bodyScrollView) {
        _bodyScrollView = [[UIScrollView alloc] initWithFrame:CGRectMake(0, 0, self.width, kAppScreenHeight)];
        ...
        //_bodyScrollView.autoresizingMask = UIViewAutoresizingNone;
